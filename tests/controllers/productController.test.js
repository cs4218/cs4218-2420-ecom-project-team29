...
describe("getProductDetailsController Unit Tests", () => {
  let req, res;
  ...
  it("should return 200 and fetch product details for valid IDs", async () => {
    req.query.ids = "65abc123ef456,65abc123ef789";
    const mockProducts = [
      { _id: "65abc123ef456", name: "Product A", price: 100 },
      { _id: "65abc123ef789", name: "Product B", price: 200 },
    ];
    ...
    expect(productModel.find).toHaveBeenCalledWith({
      _id: { $in: ["65abc123ef456", "65abc123ef789"] },
    });
  });

  it("should return 200 and fetch product details for a single valid ID", async () => {
    req.query.ids = "65abc123ef456";
    const mockProduct = [{ _id: "65abc123ef456", name: "Product A", price: 100 }];
    productModel.find.mockResolvedValue(mockProduct);

    await getProductDetailsController(req, res);

    expect(productModel.find).toHaveBeenCalledWith({ _id: {$in: ["65abc123ef456"]} });
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.send).toHaveBeenCalledWith({
      success: true,
      message: "Product details fetched successfully",
  ...
  it("should return 200 for a large number of product IDs", async () => {
    const largeProductList = Array.from({ length: 40 }, (_, i) => ({
      _id: `65abc123ef${i}`,
      name: `Product ${i}`,
      price: i * 10,
    }));

    req.query.ids = largeProductList.map((p) => p._id).join(",");
    productModel.find.mockResolvedValue(largeProductList);
  ...
  it("should return 400 if any product Id is invalid", async () => {
    req.query.ids = "invalid-id,65abc123ef456"; 
    mongoose.Types.ObjectId.isValid = jest.fn().mockImplementation((id) => id !== "invalid-id");
  ...
  it("should return 400 if all provided IDs are invalid", async () => {
    req.query.ids = "invalid-id1,invalid-id2";
  ...
  it("should return 400 errpr when ids parameter is empty", async () => {
    req.query.ids = "";
